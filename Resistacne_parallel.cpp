...
int main() {
    ...
    // Find combinations of at most four resistors in parallel
    cout << "Combinations of up to four resistors in parallel to achieve " << desiredResistance << " ohms:" << endl;
    ...
    for (size_t i = 0; i < resistances.size(); ++i) {
        ...
        for (size_t j = i + 1; j < resistances.size(); ++j) {
            double sum2 = calculateParallelResistance({resistances[i], resistances[j]});
            if (sum2 == desiredResistance) {
                cout << "(" << resistances[i] << " || " << resistances[j] << ")" << endl;
            }
            for (size_t k = j + 1; k < resistances.size(); ++k) {
                double sum3 = calculateParallelResistance({resistances[i], resistances[j], resistances[k]});
                if (sum3 == desiredResistance) {
                    cout << "(" << resistances[i] << " || " << resistances[j] << " || " << resistances[k] << ")" << endl;
                }
                for (size_t l = k + 1; l < resistances.size(); ++l) {
                    double sum4 = calculateParallelResistance({resistances[i], resistances[j], resistances[k], resistances[l]});
